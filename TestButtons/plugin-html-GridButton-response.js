var jsPsychHtmlGridButtonResponse = (function (jspsych) {
  'use strict';
  ...
  class HtmlButtonResponsePlugin {
      ...
      howManyRowsAndColumns(choices){
        // find out how many columns and the number of elements in each column
        var button_columns = choices.length;
        var button_rows = [];
        var count = 0;
        for (var i = 0; i < button_columns; i++) {
          count += choices[i].length
          button_rows.push(choices[i].length)
        }
      ...
      trial(display_element, trial) {
          ...
          this.howManyRowsAndColumns(trial.choices)
          // this makes the very first button
          ...
          // this is an index of the buttons
          for (var i = 0; i < trial.choices.length; i++) {
              if (Array.isArray(trial.choices[i])) {
                
                var trialArray = trial.choices[i]
                for (var j = 0; j < trialArray.length; j++) {
                  var str = buttons[i].replace(/%choice%/g, trialArray[j]);    
                          html +=
